...
public class testZip : MonoBehaviour
{
    ...
    void DoDecompression(){
        ...
        // extract a list of entries to newly created byte buffers
        ...
        var buffers = lzip.entries2Buffers(ppath + "/testZip.zip", ent.ToArray(), ref eprogress);
    ...
    void DoGzipBz2Tests() {
        ...
        // If we are not able to get the decompressed size of the gzip buffer we create a temp or fixed buffer where we decompress and copy from there.
        byte[] tempBuffer = new byte[1024 * 542];

        int decompressedSizeOfGzip = lzip.unGzip(bt2, tempBuffer); // lzip.unGzip2 will work too.

        plog ("Decompress gzip of unknown decompressed size: " + decompressedSizeOfGzip.ToString());
        
        if (decompressedSizeOfGzip > 0) {
            // create a buffer with the exact size of the decompressed data
            byte[] decompressedData = new byte[decompressedSizeOfGzip];
            Array.Copy(tempBuffer, decompressedData, decompressedSizeOfGzip);
            // Write to disk to verify
            // File.WriteAllBytes(ppath+"/unknownGZIP.bmp", bt3);
        ...
        // Decompress a gzip file from a byte[] buffer to file.
        gzipProgress[0] = 0;
        var inGzip = File.ReadAllBytes(ppath + "/" + "/Ftest2GZIP.bmp.gz");

        plog("ungzipBuffer2File: " + lzip.ungzipBuffer2File(inGzip, ppath+"/ungzipBuffer2File.bmp", gzipProgress).ToString() + " : " + gzipProgress[0].ToString());
        inGzip = null;
         plog();
