...
public class lzip {
        ...
        [DllImport("__Internal", CallingConvention = CallingConvention.Cdecl)]
        private static extern int zipEntries2Buffers(string zipArchive, IntPtr filenames, IntPtr buffers, int arrayLength,  IntPtr FileBuffer,int fileBufferLength, IntPtr proc, [MarshalAs(UnmanagedType.LPStr)] string password);
        ...
        [DllImport("__Internal", CallingConvention = CallingConvention.Cdecl)]
        private static extern long unGzipBuffer2File (IntPtr inBuffer, int inLength, string outFile, IntPtr progress);
        [DllImport("__Internal", CallingConvention = CallingConvention.Cdecl)]
    ...
    [DllImport(libname, EntryPoint = "zipEntries2Buffers"
    #if (UNITY_STANDALONE_WIN && ENABLE_IL2CPP) || UNITY_ANDROID
		, CallingConvention = CallingConvention.Cdecl
    #endif
    #if (UNITY_EDITOR_WIN || UNITY_STANDALONE_WIN)
    ...
    internal static extern int zipEntries2Buffers(
    #if (UNITY_STANDALONE_WIN || UNITY_EDITOR_WIN)
        [MarshalAs(UnmanagedType.LPWStr)]
    #endif
        string zipArchive, IntPtr filenames, IntPtr buffers, int arrayLength,  IntPtr FileBuffer,int fileBufferLength, IntPtr proc, [MarshalAs(UnmanagedType.LPStr)] string password);
    ...
    [DllImport(libname, EntryPoint = "unGzipBuffer2File"
    #if (UNITY_STANDALONE_WIN && ENABLE_IL2CPP) || UNITY_ANDROID
		, CallingConvention = CallingConvention.Cdecl
    #endif
    )]
    internal static extern long unGzipBuffer2File (
        IntPtr inBuffer, int inLength,
        #if (UNITY_STANDALONE_WIN || UNITY_EDITOR_WIN)
                [MarshalAs(UnmanagedType.LPWStr)]
    #endif
    string outFile, IntPtr progress);
    ...
    //
    // zipArchive       : the full path to the zip archive from which a specific file will be extracted to a byte buffer.
    // entries          : the files we want to extract to the new buffers. (If the files resides in a directory, the directory should be included.)
    // progress         : a referenced integer that will count how many entries have been extracted to the buffers.
	// FileBuffer		: A buffer that holds a zip file. When assigned the function will read from this buffer and will ignore the filePath.
    //                  : Can be a c# byte[] buffer or an unmanaged Inptr buffer or an inMemory class! When an IntPtr is used, then the zipArchive string parameter will be used to pass the Length of the Intptr buffer!
	// password			: If the archive is encrypted use a password.
    //
    ...
    public static byte[][] entries2Buffers(string zipArchive, string[] entries, ref int progress, object fileBuffer = null, string password = null) {
		...
		if(fileBuffer != null) {
            if (fileBuffer is byte[]) {
                byte[] FileBuffer = (byte[])fileBuffer;
                var fbuf = gcA(FileBuffer);
                for(int i = 0; i < len; i++) sizes[i] = (int)zipGetEntrySize(null, entries[i], fbuf.AddrOfPinnedObject(), FileBuffer.Length);
			    fbuf.Free();
            }
            if (fileBuffer is IntPtr || fileBuffer is inMemory) {
                IntPtr FileBuffer = IntPtr.Zero;
                int fileBufferLength = 0;
                if (!checkObject(fileBuffer, zipArchive, ref fileBufferLength, ref FileBuffer)) { for(int i = 0; i < len; i++) buffers[i] = null; buffers = null; return null; }
                for(int i = 0; i < len; i++) sizes[i] = (int)zipGetEntrySize(null, entries[i], FileBuffer, fileBufferLength);
            }
		} else {
            #if (!UNITY_WEBGL && !UNITY_TVOS) || UNITY_EDITOR
                for(int i = 0; i < len; i++) sizes[i] = (int)zipGetEntrySize( @zipArchive,  entries[i], IntPtr.Zero, 0);
            #endif
		}

        GCHandle[] sbuf = new GCHandle[len];

        for(int i = 0; i < len; i++) { buffers[i] = new byte[sizes[i]]; sbuf[i] = gcA(buffers[i]); }
		...
		if(fileBuffer!= null) {
            if (fileBuffer is byte[]) {
                ...
                var fbuf = gcA(FileBuffer);
                for(int i = 0; i < len; i++) {
			        res = zipEntry2Buffer(null, entries[i], sbuf[i].AddrOfPinnedObject(), sizes[i], fbuf.AddrOfPinnedObject(), FileBuffer.Length, password);
                    if (res != 1) Debug.Log ("Error extracting entry at index: " + i);
                }
			    fbuf.Free();
            }
            if (fileBuffer is IntPtr || fileBuffer is inMemory) {
                IntPtr FileBuffer = IntPtr.Zero;
                int fileBufferLength = 0;
                if (!checkObject(fileBuffer, zipArchive, ref fileBufferLength, ref FileBuffer)) {
                    for(int i = 0; i < len; i++) { buffers[i] = null; sbuf[i].Free(); }
                    buffers = null;
                    return null; 
                }
                for(int i = 0; i < len; i++) {
                    res = zipEntry2Buffer(null, entries[i], sbuf[i].AddrOfPinnedObject(), sizes[i], FileBuffer, fileBufferLength, password);
                    if (res != 1) Debug.Log ("Error extracting entry at index: " + i);
                }
            }
		} else {
            #if (!UNITY_WEBGL && !UNITY_TVOS) || UNITY_EDITOR
                for(int i = 0; i < len; i++) {
                    res = zipEntry2Buffer( @zipArchive, entries[i], sbuf[i].AddrOfPinnedObject(), sizes[i], IntPtr.Zero, 0, password);
                    if (res != 1) Debug.Log ("Error extracting entry at index: " + i);
                }
            #endif
		}

        for(int i = 0; i < len; i++) sbuf[i].Free();
	...
	public static int unGzip2(object source, byte[] outBuffer, int intPtrLength = 0) {
        ...
        int res = 0;
         ...
         if (source is IntPtr && intPtrLength > 0) {
		    ...
		    dbuf.Free();
        } else if (intPtrLength == 0 && source is IntPtr) return -11;
    ...
    public static long ungzipBuffer2File(byte[] fileBuffer, string outFile, ulong[] progress = null) {
        long res = 0;
        if(fileBuffer == null) return res;
        if(progress == null) progress = new ulong[1];

        var prog = gcA(progress);
        var fbuf = gcA(fileBuffer);
		
        res = unGzipBuffer2File(fbuf.AddrOfPinnedObject(), fileBuffer.Length, @outFile.Replace("//", "/"), prog.AddrOfPinnedObject());
